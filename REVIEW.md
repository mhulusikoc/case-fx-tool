...
- **What is wrong**: The simple in-memory cache uses `f"{base}-{target}"` as the key. It completely ignores the requested date. Furthermore, it blindly echoes back `str(on or date.today())` instead of the actual `rate_date` from the upstream payload.
...
- **How I would verify it**: Send a request with a completely invalid target currency like `?amount=100&from_=EUR&to=INVALID`. Instead of failing, the API returns a 200 OK with `result: 0.0`.
